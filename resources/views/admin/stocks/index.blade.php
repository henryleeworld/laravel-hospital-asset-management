            ...
            <table class=" table table-bordered table-striped table-hover datatable datatable-Stock">
                <thead>
                    <tr>
                        ...
                        @admin
                            <th>
                                {{ trans('cruds.stock.fields.hospital') }}
                            </th>
                        @endadmin
                        <th>
                        ...
                        @user
                            <th>
                                {{ trans('cruds.stock.actions.add') }}
                            </th>
                            <th>
                                {{ trans('cruds.stock.actions.remove') }}
                            </th>
                        @enduser
                        <th>
                            &nbsp;
                            ...
                            @user
                                <td>
                                    <form action="{{ route('admin.transactions.storeStock', $stock->id) }}" method="POST" style="display: inline-block;" class="form-inline">
                                        ...
                                        <input type="hidden" name="action" value="{{ trans('global.add') }}">
                                        <input type="number" name="stock" class="form-control form-control-sm col-4" min="1">
                                        <input type="submit" class="btn btn-xs btn-danger" value="{{ trans('global.add') }}">
                                    </form>
                                </td>
                                <td>
                                    <form action="{{ route('admin.transactions.storeStock', $stock->id) }}" method="POST" style="display: inline-block;" class="form-inline">
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                        <input type="hidden" name="action" value="{{ trans('global.remove') }}">
                                        <input type="number" name="stock" class="form-control form-control-sm col-4" min="1">
                                        <input type="submit" class="btn btn-xs btn-danger" value="{{ trans('global.remove') }}">
                                    </form>
                                </td>
                            @enduser
                            <td>
